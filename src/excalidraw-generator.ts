...
export class ExcalidrawGenerator {
  ...
  private createBaseElement(type: string, x: number, y: number, width: number, height: number): ExcalidrawElement {
    return {
      id: this.generateId(),
      type,
      x,
  ...
  private createRectangle(x: number, y: number, width: number, height: number, text?: string): ExcalidrawElement {
  ...
  private createText(x: number, y: number, text: string, fontSize: number = 16): ExcalidrawElement {
  ...
  private createArrow(startX: number, startY: number, endX: number, endY: number): ExcalidrawElement {
  ...
  private createDiamond(x: number, y: number, width: number, height: number): ExcalidrawElement {
    const element = this.createBaseElement('diamond', x, y, width, height);
    element.backgroundColor = '#fff3e0';
    return element;
  }

  private createEllipse(x: number, y: number, width: number, height: number): ExcalidrawElement {
  ...
  createSchema(description: string, type: string, elements?: any[]): ExcalidrawSchema {
    const schemaElements: ExcalidrawElement[] = [];
  ...
  private generateSimpleFlowchart(title: string): ExcalidrawSchema {
  ...
  private generateSimpleArchitecture(title: string): ExcalidrawSchema {
    const elements: ExcalidrawElement[] = [];
  ...
  private generateSimpleDatabase(title: string): ExcalidrawSchema {
    const elements: ExcalidrawElement[] = [];
  ...
  private generateNetworkDiagram(title: string): ExcalidrawSchema {
    const elements: ExcalidrawElement[] = [];
  ...
  private generateWireframe(title: string): ExcalidrawSchema {
    const elements: ExcalidrawElement[] = [];
  ...
  private generateMindMap(title: string): ExcalidrawSchema {
    const elements: ExcalidrawElement[] = [];
  ...
  private generateGenericSchema(title: string, elements?: any[]): ExcalidrawSchema {
    const schemaElements: ExcalidrawElement[] = [];
    ...
    if (elements && elements.length > 0) {
      elements.forEach((element, index) => {
        ...
        if (element.connections) {
          element.connections.forEach((connectedIndex: number) => {
  ...
  generateArchitectureDiagram(services: any[], title?: string): ExcalidrawSchema {
    ...
    const serviceElements: {[key: string]: {x: number, y: number}} = {};
    ...
    services.forEach((service) => {
      if (service.connections) {
        ...
        service.connections.forEach((targetName: string) => {
  ...
  createFlowchart(steps: any[], title?: string): ExcalidrawSchema {
    ...
    const stepElements: {[key: string]: {x: number, y: number}} = {};
  ...
  generateDatabaseSchema(tables: any[], title?: string): ExcalidrawSchema {
    ...
    const tableElements: {[key: string]: {x: number, y: number}} = {};
    ...
    tables.forEach((table, index) => {
      ...
      table.columns.forEach((column: any, colIndex: number) => {
        ...
        if (column.primaryKey) columnText += ' (PK)';
        if (column.foreignKey) columnText += ' (FK)';
  ...
  private createSchemaObject(elements: ExcalidrawElement[]): ExcalidrawSchema {
    return {
      type: 'excalidraw',
      version: 2,
